...
export default {
  ...
  created() {
    // $route.params.review에서 JSON 문자열을 파싱하여 객체로 변환
    this.review = JSON.parse(this.$route.params.review);
  },
};
</script>
...
import { modifyReview } from "@/api/foodBoard";
...
onMounted(() => {
  //URL의 params에서 review 데이터 추출
  const route = useRoute();

  console.log(route.params);
  console.log(route.params.review);
  reviewData.value = JSON.parse(route.params.review);
  //review 데이터 사용
});
...
const onClickMapMarker = (markerItem) => {
  ...
  reviewData.value.foodStoreTitle = markerItem.infoWindow.content;
  reviewData.value.foodBoardAddr = markerItem.infoWindow.addr;
...
const previewImage = (e) => {
  ...
  if (file) {
    ...
    reader.onload = (e) => {
      ...
      revieData.value.foodBoardImage = base64String;
...
const gomodify = async () => {
  console.log(reviewData.value.foodBoardNo);
  await modifyReview(
    reviewData.value,
    (response) => {
      alert("수정이 완료되었습니다.");
      router.push({
        ...
        params: {
          foodBoardNo: reviewData.value.foodBoardNo,
        },
      });
    },
    (error) => {}
        ...
        <div class="col-lg-5 col-md-4 col-sm-12">
          <fieldset>
            <div class="form-check">
              <input type="checkbox" @change="buyMyMoney" v-model="reviewData.foodBoardMyMoney" />
              <label for="buyMyMoney">내돈 내산</label>
            </div>
            <div v-if="reviewData.foodBoardMyMoney" id="showBill">
              <p>영수증 인증하기</p>
            ...
            <div class="star-rating space-x-4 mx-auto">
              <input
                ...
                name="rating"
                value="5"
                v-model="reviewData.foodBoardRank"
              />
            ...
            <div>
              <h6>선택된 주소</h6>
              <h5 class="mb-0">{{ reviewData.foodStoreTitle }}</h5>
              <p class="mt-0">{{ reviewData.foodBoardAddr }}</p>
              <!-- contentTypeId 선택 -->
              <h6>음식점 분류</h6>
              <select
                id="search-food"
                class="form-select me-2 rounded-5 select"
                v-model="reviewData.contentTypeName"
              >
            ...
            <div class="reviewBox">
              <div class="mb-3">
                ...
                <textarea
                  ...
                  rows="3"
                  v-model="reviewData.foodBoardReview"
                ></textarea>
              </div>
            </div>
            <button type="submit" @click="gomodify">저장하기</button>
          </fieldset>
        </div>
      </div>
